...
build_haul_table = function(haul_data){
  ...
  haul_data = cbind.data.frame(haul_data$EVENT_ID, haul_data$region, haul_data$GEAR, haul_data$date,
  ...
  colnames(haul_data) = c('haul', 'area', 'gear', 'date', 'time', 'duration', 'start_lat', 'start_lon',
  ...
  header_keys = data.frame(
    ...
    row1 = c('Haul', 'Area', 'Gear', 'Date', 'Time', 'Duration', 'Start Position', '','', 'Depth (m)',
             '', '', paste0('Temp (', intToUtf8(176), 'C)'), '', '', 'walleye pollock', '','', 'Other'),
    row2 = c('No.', '', 'Type', '(GMT)', '(GMT)', '(mins)', 'Lat. (N)', 'Long. (W)', '', 'Headrope',
  ...
  footnote_text_function = function(gear_type){
    ...
    if (gear_type == 'AWT') gear_name = 'Aleutian Wing Trawl'
    else if (gear_type == 'PNE') gear_name = "poly Nor'eastern bottom trawl"
    else if (gear_type == 'LFS1421') gear_name = 'LFS1421 midwater trawl'
    else if (gear_type == '83/112') gear_name = 'Bering Sea bottom trawl'
    else if (gear_type == 'Methot') gear_name = 'Methot plankton net'
    else if (gear_type == 'Marinovich') gear_name = 'Marinovich small midwater trawl'
    else if (gear_type == 'StereoDropCam') gear_name = 'camera drop on untrawlable areas.'
    else gear_name = ''
