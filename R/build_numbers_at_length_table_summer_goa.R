#' @title Create numbers at length table for MACE summer GOA cruise reports
#'
...
build_numbers_at_length_table_summer_goa <- function(biomass_nums_data) {
  ...
  MACEReports::check_input_df(template_df = check_data, input_df = biomass_nums_data)
  ...
  if (length(species_list) > 1) {
    stop(paste0(
      "biomass_nums_data contains multiple species codes: ",
  ...
  if (length(unique(biomass_nums_data$SURVEY)) > 1) {
    stop(paste0(
      "biomass_nums_data contains multiple species surveys: ",
  ...
  # build the caption
  cap_text <- paste0(
    "Pollock numbers-at-length estimates (millions of fish) by survey region during the ",
