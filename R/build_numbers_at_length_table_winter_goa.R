#' @title build_numbers_at_length_table_winter_goa
#'
...
#' \code{build_nums_at_length_table_winter_goa} function.
...
#' biomass_nums_table_list = build_numbers_at_length_table_winter_goa(
...
build_numbers_at_length_table_winter_goa = function(biomass_nums_length_data, region_name){
  ...
  check_data = template_df_numbers_biomass_at_length_tables_winter_goa
  ...
  ###########
  #1. Collect the data for biomass- and numbers- at length tables
  ...
  flextable::width(width = 10/ncol(nums_by_length_wide))%>%
  ...
  cap_text = paste0('Numbers-at-length estimates (millions of fish) from acoustic-trawl surveys of walleye pollock in the ',
                    ...
                    max(unique(nums_and_biomass_by_length$year)), ' reflect selectivity corrections for escapement of juveniles.')
