...
build_catch_table_summer_goa <- function(catch_data, specimen_data, scaling_hauls_list = NULL) {
  ...
  catch_table_data <- catch_summary %>%
    ...
    dplyr::summarize(
      TOT_WEIGHT = sum(.data$WEIGHT_IN_HAUL),
  ...
  make_table_for_gear_function <- function(gear_type) {
    ...
    catch_table_summary <- catch_table_data %>%
      ...
      dplyr::summarize(
        TOT_WEIGHT = sum(.data$TOT_WEIGHT, na.rm = TRUE),
    ...
    catch_table_summary <- cbind.data.frame(
      catch_table_summary$COMMON_NAME, catch_table_summary$SCIENTIFIC_NAME,
    ...
    colnames(catch_table_summary) <- c(
      "COMMON_NAME", "SCIENTIFIC_NAME", "TOT_WEIGHT", "percent_wt",
    ...
    cap_text <- paste0(
      "Catch by species and numbers of length and weight measurements taken from ", n_hauls,
