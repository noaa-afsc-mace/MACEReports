...
build_specimen_table_summer_goa <- function(specimen_data, scaling_hauls_list = NULL) {
  ...
  specimen_data <- specimen_data[, c(
    "HAUL", "region", "POLLOCK_CATCH_LENGTHS", "POLLOCK_WEIGHTS", "MATURITIES", "OTOLITHS",
  ...
  header_keys <- data.frame(
    ...
    row1 = c(
      "Haul", "Region", "Pollock", "Pollock", "Pollock", "Pollock", "Pollock", "Pacific capelin", "Pacific capelin",
  ...
  specimen_table <- flextable::flextable(specimen_data) %>%
    ...
    flextable::colformat_double(
      j = c(
        "POLLOCK_CATCH_LENGTHS", "POLLOCK_WEIGHTS", "MATURITIES", "OTOLITHS", "O_TAKEN",
  ...
  cap_text <- paste0(
    "Numbers of specimens measured and biological samples collected during the ",
