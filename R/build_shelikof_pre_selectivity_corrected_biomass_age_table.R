#' @title Create the old 'pre-selectivity era' biomass at age summary table for winter MACE cruise reports
...
#' }
#' @export
build_shelikof_pre_selectivity_corrected_biomass_age_table <- function() {
  ##############
  # 1. Biomass-at-age table

  # the table data is stored in /data as as pre-formatted/canned object named:
  # shelikof_historic_biomass_by_age_1981_to_2007.rda
  shelikof_historic_biomass_by_age_1981_to_2007 <- MACEReports::shelikof_historic_biomass_by_age_1981_to_2007
  ...
  biomass_age_table <- flextable::flextable(shelikof_historic_biomass_by_age_1981_to_2007) %>%
  ...
  biomass_age_table <- flextable::hline(biomass_age_table,
    i = biomass_age_table$body$content$content$nrow - 1, part = "body",
    border = table_border
  )
  ...
  years_for_caption <- as.numeric(colnames(shelikof_historic_biomass_by_age_1981_to_2007
  [2:length(colnames(shelikof_historic_biomass_by_age_1981_to_2007))]))
  # build the caption
  cap_text <- paste0(
    "Biomass-at-age estimates (thousands of metric tons) from acoustic-trawl surveys of pollock in the ",
