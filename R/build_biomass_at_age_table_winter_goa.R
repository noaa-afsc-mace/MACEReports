...
#' \code{build_biomass_at_age_table_winter_goa} function.
...
#' }
#' @export
build_biomass_at_age_table_winter_goa = function(biomass_nums_age_data, region_name){
  ...
  summary_nums_biomass_by_age = biomass_nums_age_data%>%
    ...
    #get a year from the survey name
  ...
  biomass_age_table = flextable::flextable(biomass_by_age_selectivity_corrected)%>%
    #add the footer row
    ...
    flextable::width(width = 10/ncol(biomass_by_age_selectivity_corrected))%>%
  ...
  col_keys = biomass_age_table$col_keys[2:length(biomass_age_table$col_keys)]
  ...
  biomass_age_table = flextable::set_formatter(biomass_age_table, values = formatter_func_list, part = 'body')

  #build the caption:
  #note it is hard-coded to say Shelikof Strait because that's the only region we currently make this table for

  cap_text = paste0('Biomass-at-age estimates (thousands of metric tons) from acoustic-trawl surveys ',
                    'of walleye pollock in the Shelikof Strait area. ',
                    'Numbers from ', min(unique(summary_nums_biomass_by_age$year)), '-',
