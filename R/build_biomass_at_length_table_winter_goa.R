...
build_biomass_at_length_table_winter_goa <- function(biomass_nums_length_data,
  ...
  # check that the REPORT_REGION name is present
  ...
  # use this to limit the table data to this reporting REPORT_REGION;
  # sum everyting up by length bin and SURVEY_YEAR for this REPORT_REGION
  ...
  # get the numbers data, and make the data 'wide' for the table, with each SURVEY_YEAR as a column
  biomass_by_length_selectivity_corrected <- nums_and_biomass_by_length %>%
    # keep only the columns we care about
    dplyr::select("SURVEY_YEAR", "Length", "biomass_thousand_tons") %>%
    # make data wide format for table
    tidyr::pivot_wider(id_cols = "Length", names_from = "SURVEY_YEAR", values_from = "biomass_thousand_tons")
